...
def kill_ros_nodes(ros_pref, ros_env):
    try:
        clear_ros_log(ros_pref)

        # cmd_arg = ros_pref + 'rosnode list | grep -v rosout | '
        # cmd_arg += ros_pref + 'xargs rosnode kill'
        cmd_arg = 'rosnode list | grep -v rosout | xargs rosnode kill'
        print_flush('[clear_process.py] ' + cmd_arg)
        # subprocess.call(cmd_arg, shell=True)
        clear_proc = subprocess.Popen(cmd_arg, env=dict(ros_env))
        clear_proc.wait(timeout=5)

        subprocess.call("rosnode kill recorder", env=dict(ros_env), shell=True)
    except Exception as e:
        print_flush(e)
...
class SubprocessMonitor(Process):
    def __init__(self, ros_port, verbosity=1):
        ...
        print_flush("[clear_process.py] SubprocessMonitor initialized at port {}".format(self.ros_port))
...
def clear_all(ros_port):
    ...
    ros_env['ROS_MASTER_URI'] = 'http://localhost:{}'.format(ros_port)
    kill_ros_nodes(ros_pref, ros_env)
    print_flush("[clear_process.py]  clearing process")
    print_flush("[clear_process.py] pkill -9 CarlaUE4-Linux-")
    subprocess.call('pkill -9 CarlaUE4-Linux-', shell=True)
    time.sleep(1)
    # print_flush("[clear_process.py] pkill -2 record")
    # time.sleep(2)
    # subprocess.call('pkill -9 record', shell=True)
    print_flush("[clear_process.py] pkill -9 ped_pomdp")
    subprocess.call('pkill -9 ped_pomdp', shell=True)
    
    print_flush("[clear_process.py]  clearing ros")
    ...
    print_flush("[clear_process.py] pkill -9 python3")
    subprocess.call('timeout 5 pkill -9 python3', shell=True)
    print_flush("[clear_process.py] pkill -9 python")
    subprocess.call('timeout 5 pkill -9 python', shell=True)
