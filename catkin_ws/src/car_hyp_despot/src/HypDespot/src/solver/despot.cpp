...
double SolverPrior::prior_discount_optact = 10.0;
bool SolverPrior::prior_force_acc = false;
bool SolverPrior::disable_policy_net = false;
bool SolverPrior::disable_value = true;
bool SolverPrior::use_factored_value = true;
double SolverPrior::despot_value_col_factor = 0.0;
double SolverPrior::despot_value_ncol_factor = 0.0;
int SolverPrior::belief_record_min_depth = 1;
int SolverPrior::prior_min_depth = 5;

#define COL_VALUE_THREAD -350
...
double DESPOT::state_record_prob = 0.0; // 0.1;
...
static bool record_trial = false;
...
DESPOT::DESPOT(const DSPOMDP* model, ScenarioLowerBound* lb,
	...
	if (use_GPU_) {
		PrepareGPUMemory(model, model->NumActions(), model->NumObservations());
	}

	cout << "SolverPrior::prior_min_depth = " << SolverPrior::prior_min_depth << std::endl;
}
...
VNode* DESPOT::Trial(VNode* root, RandomStreams& streams,
	...
	do {
		...
		if (!Globals::config.use_prior || SolverPrior::disable_value) {
			double start1 = clock();
			ExploitBlockers(cur);
		...
		if (DoPrint || (FIX_SCENARIO == 1 && qstar)) {
			cout.precision(4);
			cout << "thread " << 0 << " " << " trace down to QNode"
					<< " get old node" << " at depth " << cur->depth() + 1
					<< ": weight=" << qstar->Weight() << ", reward="
					<< qstar->step_reward / qstar->Weight() << ", lb="
					<< qstar->lower_bound() / qstar->Weight() << ", ub="
					<< qstar->upper_bound() / qstar->Weight() << ", uub="
					<< qstar->utility_upper_bound() / qstar->Weight()
					<< ", edge=" << qstar->edge() << ", v_loss=" << 0;
			cout << endl;
		}
		...
		if (DoPrint || (FIX_SCENARIO == 1 && next)) {
			cout.precision(4);
			cout << "thread " << 0 << " " << " trace down to VNode"
					<< " get old node" << " at depth " << next->depth()
		...
		if (next == NULL) {
			if (DoPrint)
				cout << "Debug end trial: Null next node" << endl;
			break;
		}
		...
		if (Globals::config.use_prior) {
			...
			SolverPrior::nn_priors[0]->Add_in_search(qstar->edge(),
					cur_sample_state);
		}
	...
	if (DoPrint)
		cout << "Debug end trial: WEU=" << WEU(cur) << endl;
...
Shared_VNode* DESPOT::Trial(Shared_VNode* root, RandomStreams& streams,
	...
	if (Globals::config.use_prior) {
		...
		if (threadID == 0 &&
				Random::RANDOM.NextDouble() < DESPOT::state_record_prob &&
				SolverPrior::nn_priors[0]->unlabelled_belief_.size() == 0)
			record_trial = true;
	}
	...
	do {
		...
		if (Gap(cur) == 0) {
			tout << "Trial end: gap == 0 at depth " << cur->depth() << endl;
		}
			...
			if (!cur->IsLeaf()) {
				...
				if (qstar) {
					Globals::Global_print_node(this_thread::get_id(), qstar,
							((VNode*) cur)->depth() + 1,
							((QNode*) qstar)->step_reward,
							((QNode*) qstar)->lower_bound(),
							((QNode*) qstar)->upper_bound(),
							((QNode*) qstar)->utility_upper_bound(),
							qstar->exploration_bonus,
							((QNode*) qstar)->Weight(),
							((QNode*) qstar)->edge(), -1000,
							"trace down to QNode");
					if (SolverPrior::use_factored_value && DESPOT::Print_nodes)
						tout << "factored lb: " << ((QNode*) qstar)->factored_prior_lower_bound(RWD_TOTAL)
							<< " " << ((QNode*) qstar)->factored_prior_lower_bound(RWD_NCOL)
							<< " " << ((QNode*) qstar)->factored_prior_lower_bound(RWD_COL)
							<< endl;
				}
		...
		if (Globals::config.use_prior) {
			if(!SolverPrior::disable_value) {
				...
				if (!qstar->prior_values_ready()) {
					if (qstar->parent()->depth() >= SolverPrior::prior_min_depth - 1) {
						ComputePriorValue(qstar, model);
						map<OBS_TYPE, VNode*>& children = qstar->children();
						for (map<OBS_TYPE, VNode*>::iterator it = children.begin();
									it != children.end(); it++) {
							VNode* vnode = it->second;
							InitPriorLowerBound(vnode, lower_bound, streams, history);
//							tout << "after InitPriorLowerBound " << vnode->text();
						}
						if (DESPOT::Print_nodes)
							DEBUG("Call PriorUpdate")
						PriorUpdate(qstar);
					}
				}
			}
		} else {
			if (record_trial  && cur->depth() > 0)
				ComputeNodePriorInputsOnly(cur, model);
		}
		...
		try {
			...
			if (next != NULL) {
				Globals::Global_print_node(this_thread::get_id(), next,
						...
						next->exploration_bonus, ((VNode*) next)->Weight(),
						((VNode*) next)->edge(), WEU((VNode*) next),
						"trace down to VNode");
				if (SolverPrior::use_factored_value && DESPOT::Print_nodes)
					tout << "factored lb: " << ((VNode*) next)->factored_prior_lower_bound(RWD_TOTAL)
						<< " " << ((VNode*) next)->factored_prior_lower_bound(RWD_NCOL)
						<< " " << ((VNode*) next)->factored_prior_lower_bound(RWD_COL)
						<< endl;
			}
		} catch (exception &e) {
			cout << "Locking error: " << e.what() << endl;
		...
		if (Globals::config.use_prior) {
			State* cur_sample_state = cur->particles()[0];
			SolverPrior::nn_priors[threadID]->Add_in_search(qstar->edge(),
					cur_sample_state);
		}
...
bool DESPOT::SearchEnd(double timeout, double used_time, Shared_VNode* root) {
	if (used_time >= timeout) {
		logd << "Search time out: used_time=" << used_time << endl;
		return true;
	} else if (Globals::Timeout(Globals::config.time_per_move)) {
		logd << "Global time out" << endl;
		return true;
	} else if (((VNode*) (root))->upper_bound() - ((VNode*) (root))->lower_bound() <= 1e-6) {
		logi << "Termination gap: ub " << ((VNode*) (root))->upper_bound()
				<< " lb " << ((VNode*) (root))->lower_bound() << endl;
		return true;
	}
	return false;
}

bool DESPOT::TrialEnd(Shared_VNode* cur) {
	if (cur->depth() >= Globals::config.search_depth) {
		logd << "Reach max depth: " << cur->depth() << endl;
		return true;
	} else if (WEU((VNode*) cur) <= 0) {
		logd << "WEU <=0 at depth " << cur->depth() <<
				" ub=" << cur->upper_bound(false) <<
				" lb=" << cur->lower_bound() << endl;
		return true;
	} else if (Globals::Timeout(Globals::config.time_per_move)) {
		logd << "Global timeout" << endl;
		return true;
	}
	return false;
}
...
void DESPOT::ExpandTreeServer(RandomStreams streams,
	...
	do {
		...
		if (despot_thread)
			logi
					<< "[ExpandTreeServer] Launch a despot thread from root at visit "
					<< root->visit_count_ << endl;
		;
		root->visit_count_++;
		...
		if (Expansion_done)
			...
			while (cur->parent() != NULL) {
				...
				if (Globals::config.exploration_mode == VIRTUAL_LOSS) {
					if (cur->depth() >= 0)
						qnode->exploration_bonus += CalExplorationValue(
								cur->depth());
					else
				...
				if (Globals::config.exploration_mode == VIRTUAL_LOSS) //release virtual loss
					static_cast<Shared_VNode*>(cur)->exploration_bonus +=
							CalExplorationValue(((VNode*) cur)->depth());
...
//		if (DESPOT::Debug_mode || FIX_SCENARIO == 1)
	...
	} while (!SearchEnd(timeout, used_time, root));
...
void DESPOT::ComputePriors(VNode* root, const DSPOMDP * model) {

	const std::vector<State*> particles = root->particles();

	logi << "INFO: Clearing hisoty timestamps " << endl;

	for (int prior_ID = 0; prior_ID < SolverPrior::nn_priors.size();
			prior_ID++) {
		...
		SolverPrior::nn_priors[prior_ID]->root_car_pos(car_pos.x,
				car_pos.y);
	}

	logi << "INFO: Processing root images " << endl;
	vector<VNode*> single_node_vector( { root });
	std::vector<torch::Tensor> hist_images =
			SolverPrior::nn_priors[0]->Process_node_input(root);
	std::vector<torch::Tensor> semantic =
			SolverPrior::nn_priors[0]->Process_semantic_input(single_node_vector);

	SolverPrior::nn_priors[0]->root_input(hist_images[0]);
	SolverPrior::nn_priors[0]->root_semantic_input(semantic[0]);
	
	logi << "Querying nn for root " << endl;
	bool succeed =
			SolverPrior::nn_priors[0]->Compute(hist_images, semantic, single_node_vector);
	logi << "Querying nn for root done" << endl;

	if (!succeed) {
		ERR("Root prior not computed correctly");
	}
}

void DESPOT::ComputePriorInputsOnly(VNode* root, const DSPOMDP * model) {
	const std::vector<State*> particles = root->particles();

	logi << "INFO: Clearing hisoty timestamps " << endl;

	SolverPrior::nn_priors[0]->Clear_hist_timestamps();
...
//		logi << "path_idx in root particles " << state->car.path_idx << endl;
//	}
	...
	if (use_GPU_) {
		...
		if (Globals::config.use_prior) {
			ComputePriors(root, model);
			if (SolverPrior::prior_min_depth == 0)
				InitPriorLowerBound(root, lower_bound, streams, history);
		} else
			ComputeNodePriorInputsOnly(root, model);
	}
	...
	if (statistics != NULL) {
		...
		logi << "Root bounds: (" << statistics->initial_lb << ","
	...
	cout << std::setprecision(5) << "Root preperation in " << used_time << " s"
			<< endl;
	...
	if (Globals::config.search_depth > 0) {
		if (Globals::config.use_multi_thread_) {
			...
			for (int i = 0; i < Globals::config.NUM_THREADS; i++) {
				...
				futures.push_back(
						async(launch::async, &ExpandTreeServer, local_stream,
								...
								ref(thread_backup_time[i]),
								ref(num_trials_t[i]), timeout,
								ref(Expand_queue), ref(Print_queue), i));
			}

			futures.push_back(
					async(launch::async, &PrintServer, ref(Print_queue),
							timeout));

			double passed_time = Globals::ElapsedSearchTime();
			cout << std::setprecision(5) << Globals::config.NUM_THREADS
					<< " threads started at the " << passed_time << "'th second"
					<< endl;
			try {
				while (!futures.empty()) {
			...
			cout << std::setprecision(5) << "Tree expansion in " << passed_time
					<< " s" << endl;
	...
	if (Globals::ElapsedSearchTime() < Globals::config.time_per_move * 0.9) {
		double sleep_time = Globals::config.time_per_move * 0.9
				- Globals::ElapsedSearchTime();
...
bool DESPOT::isPriorValueActive(double prior_value, VNode* vnode) {
	if (prior_value < vnode->lower_bound())
		return false;
	if (prior_value > vnode->upper_bound())
		return false;
	return true;
}

bool DESPOT::isPriorValueActive(double prior_value, QNode* qnode, bool check_bound) {
	if (! check_bound)
		return true;
	if (prior_value < qnode->lower_bound())
		return false;
	if (prior_value > qnode->upper_bound())
		return false;
	return true;
}

void DESPOT::InitPriorLowerBound(VNode* vnode, ScenarioLowerBound* lower_bound,
		RandomStreams& streams, History& history) {
	logv << __FUNCTION__ << endl;
	logv << "[InitPriorLowerBound] Using prior for lower bound value: node "
			<< vnode << " at depth " << vnode->depth() << " value "
			<< vnode->prior_value() << endl;
	...
	if (vnode->depth() == 0) {
		SolverPrior::nn_priors[prior_id]->default_action =
				vnode->max_prob_action();
	}

	if (SolverPrior::use_factored_value){
		FactoredValuedAction move;
		for (int i=0; i < RWD_NUM_FACTORS; i++) {
			move.value[i] = vnode->factored_prior_value(i) * vnode->Weight();
			move.value[i] *= Globals::Discount(vnode->depth());
		}

		if (isPriorValueActive(move.value[RWD_TOTAL], vnode)) {
			for (int i=0; i < RWD_NUM_FACTORS; i++)
				vnode->factored_prior_lower_bound(i, move.value[i]);
		} else if (DESPOT::Print_nodes)
			tout << "vnode " << vnode << " learned value blocked by the original bounds "
			<< vnode->lower_bound() << " "
...
void DESPOT::InitLowerBound(VNode* vnode, ScenarioLowerBound* lower_bound,
	...
	if (SolverPrior::use_factored_value){
		FactoredValuedAction move = lower_bound->FactoredValue(vnode->particles(), streams, history);
		logd << "[DESPOT::InitLowerBound] lb value=" << move.value <<
				" num_particles=" << vnode->particles().size() << endl;
		...
		for(int i=0; i<RWD_NUM_FACTORS; i++)
			vnode->factored_prior_lower_bound(i, move.value[i]);
		vnode->default_move(ValuedAction(move.action, move.value[RWD_TOTAL]));
		vnode->lower_bound(move.value[RWD_TOTAL]);
		logd << "[DESPOT::InitLowerBound] refracted lb value =" << move.value[RWD_TOTAL] << " " 
			<< move.value[RWD_COL] << " " << move.value[RWD_NCOL] << std::endl;
	} else {
		ValuedAction move = lower_bound->Value(vnode->particles(), streams, history);
		logd << "[DESPOT::InitLowerBound] lb value << " << move.value << endl;
...
//	if (vnode->depth()>1)
//		logi << "New node upper bound " << upper << endl;
}
...
//	tout << "after InitLowerBound " << vnode->text();

	if (DESPOT::Print_nodes) {
		tout << "[InitBounds] node " << vnode << " at level " << vnode->depth()
			<< " neural lb=" << vnode->lower_bound() << " ub="
			<< vnode->upper_bound() << endl;

		tout << "[InitBounds] node " << vnode << " at level " << vnode->depth()
				<< " refracted lb=" << vnode->factored_prior_lower_bound(RWD_TOTAL)
				<< " " << vnode->factored_prior_lower_bound(RWD_COL)
				<< " " << vnode->factored_prior_lower_bound(RWD_NCOL)
				<< endl;
	}
...
ValuedAction DESPOT::Search() {
	...
	if (FIX_SCENARIO == 1) {
		ifstream fin;
		string particle_file = "Particles" + std::to_string(step_counter)
				+ ".txt";
		...
		logi << "[FIX_SCENARIO] particles read from file " << particle_file
				<< endl;
	} else {
		if (Debug_mode)
			std::srand(0);
	...
	int stream_length = max(Globals::config.search_depth, Globals::config.max_policy_sim_len);
	...
	if (FIX_SCENARIO == 1 || Debug_mode) {
		...
		fin.close();
		cout << "[FIX_SCENARIO] random streams read from file " << stream_file
				<< endl;
	} else {
		streams = RandomStreams(Globals::config.num_scenarios, stream_length);
	}
	...
	if (ub != NULL) { // Avoid using new streams for LookaheadUpperBound
		...
		if (FIX_SCENARIO == 1 || Debug_mode) {
			std::ifstream fin;
			string stream_file = "Streams" + std::to_string(step_counter)
					+ ".txt";
	...
	logi << "[used param] time_per_move=" << Globals::config.time_per_move
			<< endl;
	...
	ValuedAction astar;
	if (Globals::config.use_prior)
		astar = OptimalActionWithPriors(root_);
	else
		astar = OptimalAction(root_);
...
void DESPOT::OutputWeight(QNode* qnode) {
	...
	if (qnode == NULL) {
		...
		logi.precision(3);
		logi << "Qnode depth: " << qnode->parent()->depth() + 1 << "  action: "
				<< qnode->edge() << "  value: "
				<< qnode->lower_bound() / qnode->Weight() << "  ub: "
				<< qnode->upper_bound() / qnode->Weight();
		logi.precision(3);
...
//		for (int action = 0; action < vnode->children().size(); action++) {
		...
		logi.precision(3);
		logi << "Vnode depth: " << vnode->depth() << "  weight: "
				<< vnode->Weight()/*<<" #particle:  "<<vnode->particles().size()*/
				<< "  value: " << vnode->lower_bound() / vnode->Weight()
				<< "  ub: " << vnode->upper_bound() / vnode->Weight()
				<< "  astar: " << astar.action << "  astar_value: "
				<< astar.value / vnode->Weight() << endl;
...
static std::vector<double> softmax(std::vector<double> input) {
	assert(input.size() >= 0);
...
ValuedAction DESPOT::OptimalActionWithPriors(VNode* vnode) {
	logv << __FUNCTION__ << endl;
	ValuedAction astar(-1, Globals::NEG_INFTY);
	QNode * best_qnode = NULL;
	...
	if (SolverPrior::use_factored_value) {
		SolverPrior::despot_value_col_factor = vnode->factored_prior_lower_bound(RWD_COL);
		SolverPrior::despot_value_ncol_factor = vnode->factored_prior_lower_bound(RWD_NCOL);

		logi << "[DESPOT::OptimalActionWithPriors] reported lb values at root: "
				<< vnode->factored_prior_lower_bound(RWD_TOTAL) << " "
				<< vnode->factored_prior_lower_bound(RWD_NCOL) << " "
				<< vnode->factored_prior_lower_bound(RWD_COL) << endl;
	}

	int root_visit = static_cast<Shared_VNode*>(vnode)->visit_count_;
	...
	for (ACT_TYPE action : vnode->legal_actions()) {
		...
		float score = qnode->factored_prior_lower_bound(RWD_TOTAL);
		q_values.push_back(score);

		if (score != EMPTY_LB_VALUE) {// only print visited nodes
			if (SolverPrior::use_factored_value)
				logi << "Children of root node: action: " << qnode->edge()
					<< " visit_count: " << visit << " upper_bound: "
					<< qnode->upper_bound() << " lower_bound: "
					<< qnode->lower_bound() << " ("
					<< qnode->factored_prior_lower_bound(RWD_TOTAL) << ","
					<< qnode->factored_prior_lower_bound(RWD_NCOL) << ","
					<< qnode->factored_prior_lower_bound(RWD_COL) << ") prior: "
					<< qnode->prior_probability() << " bonus:"
		...
		if (score > score_max) {
			astar = ValuedAction(action, score);
			best_qnode = qnode;
			score_max = score;
		}
	}

	if (Globals::config.close_loop_prior) { //training time explorative policy
		double r = Random::RANDOM.NextDouble();
		if (r <  0.0) {
			// logi << "epsilon greedy random action " << std::endl;
			// int sampled_action = Random::RANDOM.NextInt(9);
			// if (vnode->Child(sampled_action))
			// 	return ValuedAction(sampled_action, vnode->Child(sampled_action)->lower_bound());
			// else
			// 	return ValuedAction(sampled_action, -10000);

			logi << "sampling on-policy action " << std::endl;
			std::vector<double> nn_action_probs = SolverPrior::nn_priors[0]->GetPolicyProbs();
			int sampled_action = sample(nn_action_probs);
			if (vnode->Child(sampled_action))
				return ValuedAction(sampled_action, vnode->Child(sampled_action)->lower_bound());
			else
				return ValuedAction(sampled_action, -10000);
		...
		auto policy = softmax(q_values);
		logi << "despot softmax policy " << policy << std::endl;
		int sampled_idx = sample(policy);
		ACT_TYPE sampled_action = vnode->legal_actions()[sampled_idx];
		double sampled_q = vnode->Child(sampled_action)->lower_bound();

		return ValuedAction(sampled_action, sampled_q);
	}

	if (vnode->default_move().value > astar.value + 1e-5) {
		logi.precision(5);
		logi << "Searched value " << astar.value << "(act " << astar.action
				<< ") worse than default move "
				<< vnode->default_move().value << "(act "
				<< vnode->default_move().action << ")" << endl;
		astar = vnode->default_move();
		logi << "Execute default move: " << astar.action << endl;
		best_qnode = NULL;		//Debug
	} else if (astar.action != vnode->default_move().action) {
		logi << "Searched value " << astar.value << "(act " << astar.action
				<< ") better than default move "
				<< vnode->default_move().value << "(act "
				<< vnode->default_move().action << ")" << endl;
		SolverPrior::nn_priors[0]->print_prior_actions(
				vnode->default_move().action);
	} else {
		logi << "Searched value " << astar.value << "(act " << astar.action
				<< ") is the same as default move "
				<< vnode->default_move().value << "(act "
				<< vnode->default_move().action << ")" << endl;		//Debug
	}

	if (vnode->lower_bound() < ModelParams::CRASH_PENALTY * ModelParams::REWARD_BASE_CRASH_VEL) {
		logi.precision(3);
...
ValuedAction DESPOT::OptimalAction(VNode* vnode) {
	...
	if (SolverPrior::use_factored_value) {
		...
		logi << "[DESPOT::OptimalAction] reported lb values at root: "
				<< vnode->factored_prior_lower_bound(RWD_TOTAL) << " "
				<< vnode->factored_prior_lower_bound(RWD_NCOL) << " "
				<< vnode->factored_prior_lower_bound(RWD_COL) << endl;
	}
	...
	for (ACT_TYPE action : vnode->legal_actions()) {
		if (action < vnode->children().size()) {
			...
			logi << "Children of root node: action: " << qnode->edge()
					<< " visit_count: " << visit << "  lowerbound: "
					<< qnode->lower_bound() << "  upperbound: "
					<< qnode->upper_bound() << endl;
	...
	if (vnode->default_move().value > astar.value + 1e-5) {
		...
		logi << "Searched value " << astar.value << "(act " << astar.action
				<< ") better than default move "
				<< vnode->default_move().value << "(act "
				<< vnode->default_move().action << ")" << endl;	//Debug		//logi.precision(5);
...
VNode* DESPOT::SelectBestWEUNode(QNode* qnode, bool despot_thread) {
				...
				if (FIX_SCENARIO == 1) {
					if (!Globals::config.use_multi_thread_) {
						...
						cout << "thread " << 0 << " "
								...
								<< " get old node" << " at depth "
								<< vnode->depth() << ": weight="
								<< vnode->Weight() << ", reward=-" << ", lb="
								<< vnode->lower_bound() / vnode->Weight()
								<< ", ub="
								<< vnode->upper_bound() / vnode->Weight()
								<< ", uub="
								<< vnode->utility_upper_bound()
										/ vnode->Weight() << ", edge="
								<< vnode->edge() << ", v_loss=" << 0;
						if (vnode->Weight()
								== 1.0 / Globals::config.num_scenarios)
							cout << ", particle_id="
									<< "-"/*vnode->particles()[0]->scenario_id*/;
						cout << ", WEU=" << WEU(vnode);
						cout << endl;
					} else {
						...
						Globals::Global_print_node(this_thread::get_id(), vnode,
								...
								((VNode*) vnode)->Weight(),
								((VNode*) vnode)->edge(), WEU(((VNode*) vnode)),
								"  Compare children vnode");
					}
				}
			}
	...
	if (vstar && Globals::config.use_multi_thread_) {
		...
		case VIRTUAL_LOSS:
			static_cast<Shared_VNode*>(vstar)->exploration_bonus -=
					CalExplorationValue(vstar->depth());
			break;
		case UCT:
...
Shared_QNode* DESPOT::SamplePriorPolicyNode(Shared_VNode* vnode) {
	int astar = -1;
	double probstar = Globals::NEG_INFTY;
	bool use_exploration_value = true;
	...
	for (ACT_TYPE action : vnode->legal_actions()) {
		...
		if (use_exploration_value) {
			...
			CalPriorExplorationValue(qnode);

//			tout << "comparing level " << qnode->parent()->depth() << " qnode "
//					<< qnode << " , prior_action_probs()= " <<
//					vnode->prior_action_probs(action, false)
//					<< " bonus " << qnode->exploration_bonus << endl;
		}

		if (vnode->prior_action_probs(action, use_exploration_value) > probstar + 1e-5) {
			probstar = vnode->prior_action_probs(action, use_exploration_value);
			astar = action;
		}
	}
	...
	qstar->visit_count_++;
	CalPriorExplorationValue(qstar);
...
Shared_QNode* DESPOT::SelectBestUpperBoundNode(Shared_VNode* vnode,
	...
	for (ACT_TYPE action : vnode->legal_actions()) {
		...
		if (qnode && Globals::config.use_multi_thread_
		...
		logv << "comparing level " << qnode->parent()->depth() << " qnode "
				<< qnode << " , upper_bound()= " << qnode->upper_bound(false)
				<< " , lower_bound()= " << qnode->lower_bound() << " bonus "
				<< qnode->exploration_bonus << ", ";
	...
	if (Globals::config.exploration_mode == VIRTUAL_LOSS)
		qstar->exploration_bonus -= CalExplorationValue(
				((VNode*) vnode)->depth() + 1);
...
void DESPOT::Update(Shared_VNode* vnode, bool real) {
	...
	if (((VNode*) vnode)->depth() > 0 && real) {
		if (Globals::config.exploration_mode == VIRTUAL_LOSS) //release virtual loss
			vnode->exploration_bonus += CalExplorationValue(
					((VNode*) vnode)->depth());
		else if (Globals::config.exploration_mode == UCT) //release virtual loss
			vnode->exploration_bonus += CalExplorationValue(
					((VNode*) vnode)->depth()) * ((VNode*) vnode)->Weight();
	}
...
void DESPOT::PriorUpdate(VNode* vnode) {
	double lower = EMPTY_LB_VALUE + 1;
	QNode* qstar = NULL;
	for (ACT_TYPE action : vnode->legal_actions()) {
		Shared_QNode* qnode =
				static_cast<Shared_QNode*>(((VNode*) vnode)->Child(action));
		if(qnode==NULL)
			ERR("[DESPOT::PriorUpdate] qnode can not be NULL");

		if (qnode->factored_prior_lower_bound(RWD_TOTAL) > lower) {
			qstar = (QNode*) qnode;
			lower = qstar->factored_prior_lower_bound(RWD_TOTAL);
		}
	}

	if (qstar == NULL)
		ERR("[DESPOT::PriorUpdate] qstar can not be NULL");

	if (isPriorValueActive(lower, vnode))
		for (int i = 0; i < RWD_NUM_FACTORS; i++) {
			double lb_star = qstar->factored_prior_lower_bound(i);
			vnode->factored_prior_lower_bound(i, lb_star);
		}

//	tout << "after PriorUPdate " << vnode->text();
}

void DESPOT::PriorUpdate(QNode* qnode, bool check_bound) {
	double lower = qnode->step_reward;
	double lower_col_factor = qnode->step_reward_col_factor;
	double lower_ncol_factor = qnode->step_reward_ncol_factor;
	if (DESPOT::Print_nodes)
		tout << "before PriorUpdate " << qnode->text();

	map<OBS_TYPE, VNode*>& children = ((QNode*) qnode)->children();
	for (map<OBS_TYPE, VNode*>::iterator it = children.begin();
			it != children.end(); it++) {
		Shared_VNode* vnode = static_cast<Shared_VNode*>(it->second);
		if (vnode->factored_prior_lower_bound(RWD_TOTAL) <= EMPTY_LB_VALUE + 1)
			ERR("[DESPOT::PriorUpdate] vnode factored_lb cannot be EMPTY_LB_VALUE");

		lower += ((VNode*) vnode)->factored_prior_lower_bound(RWD_TOTAL);
		lower_col_factor += ((VNode*) vnode)->factored_prior_lower_bound(RWD_COL);
		lower_ncol_factor += ((VNode*) vnode)->factored_prior_lower_bound(RWD_NCOL);
	}

	if (isPriorValueActive(lower, qnode, check_bound)) {
		((QNode*) qnode)->factored_prior_lower_bound(RWD_TOTAL, lower);
		((QNode*) qnode)->factored_prior_lower_bound(RWD_COL, lower_col_factor);
		((QNode*) qnode)->factored_prior_lower_bound(RWD_NCOL, lower_ncol_factor);
	}

	if (DESPOT::Print_nodes)
		tout << "after PriorUpdate " << qnode->text();
}
		...
		if (Globals::config.use_multi_thread_) {
			...
			if (real) {
				Globals::Global_print_node(this_thread::get_id(), vnode,
						...
						static_cast<Shared_VNode*>(vnode)->upper_bound(
								display_explore_value), -1000,
						static_cast<Shared_VNode*>(vnode)->GetVirtualLoss(),
						static_cast<Shared_VNode*>(vnode)->Weight(),
						static_cast<Shared_VNode*>(vnode)->edge(), -1000,
						msg.c_str());
				if (SolverPrior::use_factored_value && DESPOT::Print_nodes)
					tout << "factored lb: " << vnode->factored_prior_lower_bound(RWD_TOTAL)
						<< " " << vnode->factored_prior_lower_bound(RWD_NCOL)
						<< " " << vnode->factored_prior_lower_bound(RWD_COL)
						<< endl;
			}
			...
			if (FIX_SCENARIO == 1) {
				cout.precision(4);
				cout << "thread " << 0 << " " << msg << " get old node "
						<< vnode << " at depth " << vnode->depth()
						<< ": weight=" << vnode->Weight() << ", reward=" << "--"
						<< ", lb=" << vnode->lower_bound() / vnode->Weight()
						<< ", ub=" << vnode->upper_bound() / vnode->Weight()
						<< ", edge=" << vnode->edge() << ", v_loss=" << 0;
				cout << endl;
			}
		}
		...
		if (Globals::config.use_multi_thread_) {
			...
			if (real) {
				Globals::Global_print_node<int>(this_thread::get_id(), parentq,
						...
						static_cast<Shared_QNode*>(parentq)->upper_bound(
								display_explore_value), -1000,
						static_cast<Shared_QNode*>(parentq)->GetVirtualLoss(),
						static_cast<Shared_QNode*>(parentq)->Weight(),
						static_cast<Shared_QNode*>(parentq)->edge(), -1000,
						msg.c_str());
				if (SolverPrior::use_factored_value && DESPOT::Print_nodes)
					tout << "factored lb: " << parentq->factored_prior_lower_bound(RWD_TOTAL)
						<< " " << parentq->factored_prior_lower_bound(RWD_NCOL)
						<< " " << parentq->factored_prior_lower_bound(RWD_COL)
						<< endl;
			}
		} else {
			Update(parentq, real);
			if (FIX_SCENARIO == 1) {
				cout.precision(4);
				cout << "thread " << 0 << " " << msg << " get old node "
						<< parentq << " at depth " << vnode->depth()
						<< ": weight=" << parentq->Weight() << ", reward="
						<< parentq->step_reward / parentq->Weight() << ", lb="
						<< parentq->lower_bound() / parentq->Weight() << ", ub="
						<< parentq->upper_bound() / parentq->Weight()
						<< ", edge=" << parentq->edge() << ", v_loss=" << 0;
				cout << endl;
			}
		}
...
void DESPOT::ComputeLegalActions(VNode* vnode, const DSPOMDP * model) {
	...
	vnode->legal_actions(
			SolverPrior::nn_priors[prior_ID]->ComputeLegalActions(
			static_cast<Shared_VNode*>(vnode), state, model));

	logv << " with " << vnode->legal_actions().size() << " legal actions"
				<< endl;
}
...
void DESPOT::ComputePriorPref(VNode* vnode, const DSPOMDP * model) {
	...
	logv << "[ComputePriorPref] for actions under vnode " << vnode << endl;
	...
	vector<VNode*> single_node_vector( { vnode });

	if (SolverPrior::disable_policy_net) {
		SolverPrior::nn_priors[prior_ID]->SetDefaultPriorPolicy(single_node_vector);
		return;
	}

	// Process last 3 steps of history
	logv << "INFO: Processing history images " << endl;
	bool record_unlabbeled = false;
	int record_child = -1;
	if (Globals::config.state_source == STATE_FROM_SIM)
		if (recordUnlabelledbelief(prior_ID, vnode->depth())) {
			record_unlabbeled = true;
			record_child = 0;
		}

	std::vector<torch::Tensor> nn_input =
			SolverPrior::nn_priors[prior_ID]->Process_node_input(vnode, record_unlabbeled); // 1 = PARTIAL
	std::vector<torch::Tensor> semantic =
			SolverPrior::nn_priors[prior_ID]->Process_semantic_input(
					single_node_vector, record_unlabbeled, record_child);
	// Query the nn and initialize the priors in collected nodes
	logv << "INFO: Querying nn for prior probabilities of the vnode"
			<< endl;
	SolverPrior::nn_priors[prior_ID]->ComputePreference(nn_input, semantic, single_node_vector);
}

void DESPOT::ComputeNodePriorInputsOnly(VNode* vnode, const DSPOMDP * model) {
	logv << __FUNCTION__ << endl;

	if (vnode->depth() == 0 || Globals::config.state_source == STATE_FROM_SIM) {
		int thread_ID = 0;
		if (Globals::config.use_multi_thread_) {
			thread_ID = MapThread(this_thread::get_id());
		}

		int receiver_prior_ID = max(0, vnode->depth() - SolverPrior::belief_record_min_depth);
		if (vnode->depth() == 0 ||
				(thread_ID == 0 && receiver_prior_ID < SolverPrior::nn_priors.size())) {
			cout << "[ComputeNodePriorInputsOnly] for vnode " << vnode << " of depth "
					<< vnode->depth() << endl;
			vector<VNode*> single_node_vector( { vnode });
			...
			bool record_unlabbeled = false;
			int record_child = -1;
			if (vnode->depth() >= SolverPrior::belief_record_min_depth) {
				record_unlabbeled = true;
				record_child = 0;
			}

			std::vector<torch::Tensor> hist_images =
					SolverPrior::nn_priors[receiver_prior_ID]->Process_node_input(
							vnode, record_unlabbeled); // 1 = PARTIAL
			std::vector<torch::Tensor> semantic =
					SolverPrior::nn_priors[receiver_prior_ID]->Process_semantic_input(
							single_node_vector, record_unlabbeled, record_child);
...
bool DESPOT::recordUnlabelledbelief(int prior_ID, int depth) {
	return depth >= SolverPrior::belief_record_min_depth
			&& Random::RANDOM.NextDouble() < DESPOT::state_record_prob
			&& SolverPrior::nn_priors[prior_ID]->unlabelled_belief_.size() == 0;
}

void DESPOT::ComputePriorValue(QNode* qnode, const DSPOMDP * model) {
	logv << __FUNCTION__ << endl;
	int prior_ID = 0;
	if (Globals::config.use_multi_thread_) {
		prior_ID = MapThread(this_thread::get_id());
	...
	logv << "[ComputePriorValue] for chilren vnodes of qstar " << qnode
			<< endl;
	...
	for (map<OBS_TYPE, VNode*>::iterator it = children.begin();
			it != children.end(); it++) {
		int rand_id = Random::RANDOM.NextInt(
				it->second->particles().size());
	...
	if (SolverPrior::disable_value) {
		SolverPrior::nn_priors[prior_ID]->SetDefaultPriorValue(vnode_collect);
		return;
	}

	// Process into the bach of tensors
	logv << "INFO: Processing node images " << endl;
	bool record_unlabbeled = false;
	int record_child = -1;
	if (Globals::config.state_source == STATE_FROM_SIM)
		if (recordUnlabelledbelief(prior_ID, vnode_collect[0]->depth())) {
			record_unlabbeled = true;
			record_child = Random::RANDOM.NextInt(children.size());
//			logi << "recording particles for child " << record_child << endl;
		}

	std::vector<torch::Tensor> nn_nodes_batch =
			SolverPrior::nn_priors[prior_ID]->Process_nodes_input(
					vnode_collect, node_states, record_unlabbeled, record_child);
	std::vector<torch::Tensor> nn_semantic_batch =
			SolverPrior::nn_priors[prior_ID]->Process_semantic_input(
					vnode_collect, record_unlabbeled, record_child);
	// Query the nn and initialize the priors in colleccted nodes

	logv << "INFO: Querying nn for initial value of nodes " << endl;
	SolverPrior::nn_priors[prior_ID]->ComputeValue(nn_nodes_batch,
			nn_semantic_batch, vnode_collect);

	qnode->prior_values_ready(true);
}
...
void DESPOT::Expand(VNode* vnode, ScenarioLowerBound* lower_bound,
	...
	if (use_GPU_ && !vnode->PassGPUThreshold()) {
		...
		if (particles[0] == NULL) // switching point
		{
			...
			if (Globals::MapThread(this_thread::get_id()) == 0) {
				logv << " Read-back GPU particle at depth " << vnode->depth()
						<< ": " << endl; //Debugging
			}
		}
	...
	if (!use_GPU_ || !vnode->PassGPUThreshold())
		Globals::Global_print_expand(this_thread::get_id(), vnode,
				vnode->depth(), vnode->edge());
	...
	for (ACT_TYPE action : vnode->legal_actions()) {
		...
		if (Globals::config.use_prior) {
			qnode->prior_probability( vnode->prior_action_probs(action) );
		}
	...
	if (use_GPU_ && vnode->PassGPUThreshold())
		...
		for (ACT_TYPE action : vnode->legal_actions()) {
			...
			if (SolverPrior::use_factored_value)
				FRExpand(qnode, lower_bound, upper_bound, model, streams, history);
			else
				Expand(qnode, lower_bound, upper_bound, model, streams, history);

//			tout << "after FRExpand " << qnode->text();
		}
		HitCount++;
	}
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
	...
	logv << "qnode " << qnode << " has " << NumParticles << " particles"
			<< endl;
	...
	for (int i = 0; i < NumParticles; i++) {
		...
		ContextPomdp::SetPathID(copy, static_cast<Shared_QNode*>(qnode));
		assert(copy != NULL);
	...
	for (map<OBS_TYPE, vector<State*> >::iterator it = partitions.begin();
		...
		logv << " New node created with " << vnode->legal_actions().size()
				<< " legal actions!" << endl;
		children[obs] = vnode;
	}
...
/// Expand function with factored reward
void DESPOT::FRExpand(QNode* qnode, ScenarioLowerBound* lb,
		ScenarioUpperBound* ub, const DSPOMDP* model, RandomStreams& streams,
		History& history) {
	logv << __FUNCTION__ << endl;
	...
	double step_reward[3] = {0, 0, 0};

	// Partition particles by observation
	map<OBS_TYPE, vector<State*> > partitions;
	OBS_TYPE obs;
	double reward[3] = {0, 0, 0};
	auto start = Time::now();
	int NumParticles = particles.size();
	...
	for (int i = 0; i < NumParticles; i++) {
		...
		for (int i = 0; i < model->NumRewardFactors(); i++)
			step_reward[i] += reward[i] * copy->weight;

		logv << " After step: " << *copy << " " << (reward[RWD_TOTAL] * copy->weight)
				<< " " << reward[RWD_TOTAL] << " " << copy->weight << endl;
	...
	for (int i = 0; i < model->NumRewardFactors(); i++)
		step_reward[i] = Globals::Discount(parent->depth()) * step_reward[i];
	step_reward[RWD_TOTAL] -= Globals::config.pruning_constant;	//pruning_constant is used for regularization
	step_reward[RWD_NCOL] -= noncol_value_transform(Globals::config.pruning_constant);	//pruning_constant is used for regularization

	qnode->step_reward = step_reward[RWD_TOTAL];
	qnode->step_reward_col_factor = step_reward[RWD_COL];
	qnode->step_reward_ncol_factor = step_reward[RWD_NCOL];
	...
	if (FIX_SCENARIO == 1 || doPrint) {
		...
		if (qnode->edge() == 0)
			cout << endl;
		cout << "step reward (d= " << parent->depth() + 1 << " ): "
				<< step_reward[RWD_TOTAL] / parent->Weight() << endl;
	}
...
void DESPOT::InitChildrenBounds(QNode* qnode, ScenarioLowerBound* lb,
	...
	if (FIX_SCENARIO == 1 || DESPOT::Print_nodes) {
		cout.precision(10);
		cout << " [CPU Qnode] New qnode's bounds: (d= "
				<< qnode->parent()->depth() + 1 << " ,action=" << qnode->edge()
		...
		if (SolverPrior::use_factored_value)
			tout << "factored lb: " << qnode->factored_prior_lower_bound(RWD_TOTAL)
				<< " " << qnode->factored_prior_lower_bound(RWD_NCOL)
				<< " " << qnode->factored_prior_lower_bound(RWD_COL)
				<< endl;
	}
}
