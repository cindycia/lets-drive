...
public:
	...
	Shared_VNode(int count, double value, int depth = 0, Shared_QNode* parent = NULL,
		OBS_TYPE edge = (OBS_TYPE)-1);
	void InitPathIDs(Shared_QNode* parent);
	~Shared_VNode();
	...
	double prior_action_probs(int i, bool use_Vloss);
...
public:
	...
	at::Tensor map_tensor;
  	...
  	void path_id(int laneID, int id) {
  		path_ids_[laneID] = id;
  	}
  	int path_id(int laneID) {
  		return path_ids_[laneID];
  	}
...
private:
	void InitPathID(ACT_TYPE edge, Shared_VNode* parent);
};
...
#define EMPTY_LB_VALUE -1000000.0
