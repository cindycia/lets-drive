...
public:
	...
	/**
		 * [Optional]
		 * Simulative model for refracted reward model
		 * Returns whether the terminal state has been reached
		 * @param state      Current state of the world in a scenario
		 * @param action     Action to be taken
		 * @param reward     Reward received after taking action from state
		 * @param obs        Observation received after taking action
		 */
	virtual double CollReward(State& state, double random_num, ACT_TYPE action) const {
		throw "This function should not be reached!";
	}

	virtual double NonCollReward(State& state, double random_num, ACT_TYPE action) const {
		throw "This function should not be reached!";
	}

	virtual bool Step(State& state, double random_num, ACT_TYPE action, double reward[],
			OBS_TYPE& obs) const {
		throw "This function should not be reached!";
	}

	virtual int NumRewardFactors() const {
		throw "This function should not be reached!";
	}
