...
protected:
  std::vector<State*> particles_; // Used in DESPOT
	...
	double lower_bound_;
	double prior_lower_bound_;
	double prior_lower_bound_col_factor_;
	double prior_lower_bound_ncol_factor_;
...
protected:
  	...
  	double prior_value_;
  	double prior_value_col_factor_;
  	double prior_value_ncol_factor_;
...
public:
	...
	double prior_value();
  	...
  	void factored_prior_value(int idx, double v) {
  		switch (idx) {
  			case RWD_TOTAL: prior_value_ = v; break;
  			case RWD_NCOL: 	prior_value_ncol_factor_ = v; break;
  			case RWD_COL: 	prior_value_col_factor_ = v; break;
  			default:  throw "prior value index over-flow in VNode!";
  			}}

  	double factored_prior_value(int idx) {
		switch (idx) {
		case RWD_TOTAL: return prior_value_;
		case RWD_NCOL: 	return prior_value_ncol_factor_;
		case RWD_COL: 	return prior_value_col_factor_;
		default:  throw "prior value index over-flow in VNode!";
		}}
...
public:
	...
	void factored_prior_lower_bound(int idx, double value);
	double factored_prior_lower_bound(int idx);
...
public:
	...
	double utility_upper_bound_;
	double step_reward;
	double step_reward_col_factor;
	double step_reward_ncol_factor;
	double likelihood;
