...
public:
	...
	void ConnectCarla();
	bool IsInMap(const CarStruct& pos, bool insearch=false);
	bool ValidForLaneChange(const CarStruct &car);
	Path* ParseLanePath(COORD pos, double yaw, int laneID) const;
	std::vector<Path> ParsePathCandidates(std::string edge, int lane, int segment, float offset, std::string);
	bool ParseLanePath(despot::Shared_VNode* vnode, const PomdpState* state, int laneID) const;
	bool ExtendCurLane(const PomdpState* state) const;
	bool LaneExist(COORD pos, double yaw, int laneID) const;
	void ExtendPath(const COORD car_pos, Path& path, double desired_length);

public: 
	// lane changing related
	static PathTree* path_tree_;
	PathTree* ResetPathTree();
	std::string GetLaneText(LaneCode code) const;
	const Path* GetPath(int path_idx) const;

	int last_decision_lane;
	COORD last_lane_change_car_pos;
	double last_lane_change_yaw;

	std::vector<int> SetRootPathIDs(Shared_VNode* root);
	bool LaneChangedAfterRoot(int path_idx) {return (path_idx != 0);}
	
  	double GetSteerFromLane(CarStruct &car, double lane);

  	std::map<int, int> root_path_id_map;
