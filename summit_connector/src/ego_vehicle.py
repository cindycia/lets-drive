...
class EgoVehicle(Summit):
    def __init__(self):
        ...
        # self.exclude_crowd_range = rospy.get_param('~exclude_crowd_range', 0.0)  # DEBUGGING!!!
        ...
        # else: 
        #     self.pomdp_lane_decision_sub = rospy.Subscriber('/pomdp_cmd_lane', Int32,
        #                                                 self.gamma_lane_change_decision_callback, queue_size=1)
        ...
        self.last_lane_pub = rospy.Publisher('/last_lane_decision', Int32, queue_size=1)
        ...
        while not agent_ready:
            ...
            try:
                msg = rospy.wait_for_message("/agents_ready", Bool, timeout=1)  # wait for agents to initialize
                agent_ready = True
            except Exception as e:
                pass
        print("Ego-vehicle start driving")
        sys.stdout.flush()
    ...
    def update_crowd_range(self):
        ...
        if self.last_crowd_range_update is None or time.time() - self.last_crowd_range_update > 1.0:
            ...
            try:
                ...
                print("[ego_vehicle.py:update_crowd_range] {}".format(e))
                sys.stdout.flush()
    ...
    def publish_plan(self):
        ...
        last_lane = Int32()
        last_lane.data = self.lane_decision
        self.last_lane_pub.publish(last_lane)
    ...
    def send_control_from_vel(self):
        ...
        if self.control_mode == 'gamma':
            ...
            cmd_speed = min(self.imitation_cmd_speed, self.gamma_max_speed)
    ...
    def update_path(self, lane_decision):
        ...
        try:
            ...
            cur_route_point = self.sumo_network.get_nearest_route_point(ego_veh_pos)
            if len(self.sumo_network.get_next_route_points(cur_route_point, 1.0)) == 1:
                next_rp_along_lane = self.sumo_network.get_next_route_points(cur_route_point, 1.0)[0]
                forward_vec = self.sumo_network.get_route_point_position(next_rp_along_lane) - \
                    self.sumo_network.get_route_point_position(cur_route_point)
            else:
                forward_vec = carla.Vector2D(math.cos(yaw), math.sin(yaw))
            sidewalk_vec = forward_vec.rotate(np.deg2rad(90))  # rotate clockwise by 90 degree
    ...
    def update(self):
        try:
            ...
            # self.draw_topic_path(self.path_from_topic)
            self.update_crowd_range()
            self.publish_odom()
            # self.publish_plan()
        except Exception as e:
