...
from msg_builder.msg import SimpleRountPoint
...
class CrowdProcessor(Summit):
    def __init__(self):
        ...
        self.agents_path_pub = rospy.Publisher(
            ...
            queue_size=1)
        self.world_agents_pub = rospy.Publisher(
            '/world_agents',
            msg_builder.msg.WorldAgents,
            queue_size=1)
    ...
    def update(self):
        ...
        if not self.ego_car_info:
            print_flush("[crowd_processor.py] waiting for ego car state")
            time.sleep(1)
            return

        if len(self.world.get_actors()) > self.total_num_agents / 1.2 or time.time() -start_time > 15.0:
            # print_flush("[crowd_processor.py] {} crowd agents ready".format(
                # len(self.world.get_actors())))
            self.agents_ready_pub.publish(True)
        else:
            pass
            # print_flush("[crowd_processor.py] {} percent of agents ready".format(
        ...
        for actor in self.world.get_actors():
            ...
            if local_intention[0] in ['Car', 'Bicycle']:
                ...
                agent_paths_tmp.path_start_rp = SimpleRountPoint()
                agent_paths_tmp.path_start_rp.edge = initial_route_point.edge
                agent_paths_tmp.path_start_rp.lane = initial_route_point.lane
                agent_paths_tmp.path_start_rp.segment = initial_route_point.segment
                agent_paths_tmp.path_start_rp.offset = initial_route_point.offset
                ...
                agent_paths_tmp.path_start_rp = SimpleRountPoint()
                agent_paths_tmp.path_start_rp.lane = sidewalk_route_point.polygon_id
                agent_paths_tmp.path_start_rp.segment = sidewalk_route_point.segment_id
                agent_paths_tmp.path_start_rp.offset = sidewalk_route_point.offset
        ...
        try:
            ...
            world_agents_msg = msg_builder.msg.WorldAgents()
            world_agents_msg.agents = agents_msg
            world_agents_msg.agent_paths = agents_path_msg
            self.world_agents_pub.publish(world_agents_msg)
        ...
        end_time = rospy.Time.now()
        elapsed = (end_time - init_time).to_sec()
        # print_flush('agent_array update = {} ms = {} hz'.format(duration * 1000, 1.0 / duration))
        # print_flush('agent_array update at {}'.format(elapsed))
        # sys.stdout.flush()
