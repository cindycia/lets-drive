...
struct CarStruct {
	...
	int path_idx; // idx in path_tree.

	CarStruct(): pos(0, 0), vel(0), heading_dir(0), path_idx(-1) {}

//	void Text() const {
//		std::cout << "CarStruct: pos / heading / vel / path_idx = " <<
//				pos.x << " " << pos.y << " / " << heading_dir <<
//				" / " << path_idx << endl;
//	}

	std::string Text() const {
		stringstream ss;
		ss << "CarStruct: pos / heading / vel / path_idx = " <<
				pos.x << " " << pos.y << " / " << heading_dir <<
				" / " << path_idx;
		return ss.str();
	}
...
public:
	...
	void assign(const PomdpState& src){
		car = src.car;
		num = src.num;
		for (int i = 0; i < num; i++){
			agents[i] = src.agents[i];
		}
		time_stamp = src.time_stamp;
	}
};
