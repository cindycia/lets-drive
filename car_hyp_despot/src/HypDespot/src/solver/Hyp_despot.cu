...
void DESPOT::PrepareGPUMemory(const DSPOMDP* model, int num_actions,
	...
	for (int i = 0; i < num_copies; i++) {
		...
		logi <<"Dvc_particleIDs_long[i]: "<< Dvc_particleIDs_long[i] << ",Globals::config.num_scenarios="<< Globals::config.num_scenarios<<endl;
...
void DESPOT::CalPriorExplorationValue(Shared_QNode* node){
	double node_weight = ((QNode*)node)->Weight();
	double parent_weight = ((QNode*)node)->parent()->Weight();
	double parent_visit = static_cast<Shared_VNode*>(((QNode*)node)->parent())->visit_count_;
	double node_nscene = max( node_weight * Globals::config.num_scenarios,1.1);
	double parent_nscene = max( parent_weight * Globals::config.num_scenarios,1.1);

	if(Globals::config.exploration_constant>0){
		node->exploration_bonus = Globals::config.exploration_constant * node->upper_bound(false) *
				sqrt(parent_visit* parent_nscene / ((node->visit_count_* node_nscene) + 1));
	}
}
	...
	if(Globals::config.exploration_constant>0){
		if(!Globals::config.use_prior){// UCT
			node->exploration_bonus = Globals::config.exploration_constant *
				sqrt(log(parent_visit*parent_nscene)/(node->visit_count_*node_nscene));

		}else {// PUCT
			node->exploration_bonus= Globals::config.exploration_constant *
					node->prior_probability() * sqrt(parent_visit*parent_nscene
					/ ((node->visit_count_* node_nscene) + 1));
		}
...
void DESPOT::PrepareGPUDataForNode(VNode* vnode,const DSPOMDP* model, int ThreadID,RandomStreams& streams)
{
	...
	logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " Copy particle IDs to GPU" << endl;
		...
		const DSPOMDP* model, RandomStreams& streams,History& history, bool Do_rollout)
{
	...
	if(Obs_type==OBS_INT_ARRAY)
	{
		logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " step kernels" << endl;
	...
	logi << "Thread " << ThreadID << ": [DESPOT::MCSimulation] Rollout GPU particles "<< endl;
	...
	logi << "Thread " << ThreadID << ": [DESPOT::MCSimulation] Read back GPU data "<< endl;
	...
	logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " End" << endl;
}
...
void DESPOT::GPU_Expand_Action(VNode* vnode, ScenarioLowerBound* lb,
	...
	logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " Create CPU nodes" << endl;
...
void DESPOT::GPU_InitBounds(VNode* vnode, ScenarioLowerBound* lower_bound,
	...
	cout << std::setprecision(5) << "Root PrepareGPUDataForNode in " << used_time << " s" << endl;*/
	...
	logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " Start roll-out" << endl;
	...
	logi << "Thread " << ThreadID << ": [" << __FUNCTION__ << "]" << " Readback data" << endl;
	...
	if (used_time > 0.2){
		cout << "Over-long GPUInitBound! Exiting.\n";
		exit(1);
	}
