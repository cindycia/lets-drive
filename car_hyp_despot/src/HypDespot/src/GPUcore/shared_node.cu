	...
	OBS_TYPE edge)
{
	...
	prior_lower_bound_ = EMPTY_LB_VALUE;
...
void Shared_VNode::InitPathIDs(Shared_QNode* parent) {
	logd << "Init path ids for vnode" << endl;
	path_ids_.resize(ModelParams::NumLaneDecisions);
	std::fill(path_ids_.begin(), path_ids_.end(), -1);

	if (parent != NULL)
		path_ids_[LaneCode::KEEP] = parent->path_id;
}
...
Shared_QNode::Shared_QNode(Shared_VNode* parent, ACT_TYPE edge) {
	...
	lower_bound_ = EMPTY_LB_VALUE;
	prior_lower_bound_ = EMPTY_LB_VALUE;
	prior_lower_bound_col_factor_ = EMPTY_LB_VALUE;
	upper_bound_ = EMPTY_UB_VALUE;

	InitPathID(edge, parent);
}
...
void Shared_QNode::InitPathID(ACT_TYPE edge, Shared_VNode* parent) {
	int laneID = ContextPomdp::GetLaneID(edge);
	path_id = parent->path_id(laneID);
}
...
double Shared_VNode::prior_action_probs(int i, bool use_Vloss) {
	if(use_Vloss)
		return prior_action_probs_[i] + exploration_bonus;
	else
		return prior_action_probs_[i];
}
} // namespace despot
