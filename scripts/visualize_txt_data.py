    ...
    with open(txt_file, 'r') as f:
        for line in f:
            ...
            if 'Round 0 Step' in line:
                ...
                cur_step = int(literal_eval(line_split.split('-', 1)[0]))
            ...
            try:
                ...
                if "car pos / heading / vel" in line:  # ego_car info
                    ...
                    exo_count = int(literal_eval(line.split(' ')[0]))
                    exos_list[cur_step] = []
                    ...
                    agent_id = int(literal_eval(line_split[16+1]))
                    ...
                    # (x, y, heading in rad)
                    line_split = line.split(' ')
                    pred_step = int(literal_eval(line_split[0][14:]))
                    ...
                    line_split = line.split(' ')
                    pred_step = int(literal_eval(line_split[0][17:]))
                    if pred_step == 0:
                    ...
                    line_split = line.split(' ')
                    trial = int(literal_eval(line_split[6]))
                    depth = int(literal_eval(line_split[8]))
...
def animate(time_step):
    ...
    for agent_dict in exos_list[time_step]:
        ...
        patches.append(ax.add_patch(
            vel_arrow(agent_dict, ego_pos, 'darkred')))

    # draw path
    if time_step in ego_path_list.keys():
...
if __name__ == "__main__":
    ...
    num_frames = len(ego_list.keys()) - config.frame

    anim = animation.FuncAnimation(fig, animate, init_func=init,
                               frames=num_frames, interval=config.int, blit=True, repeat=False)
